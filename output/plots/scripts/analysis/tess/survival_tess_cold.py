"""
12_survival_tess_cold.py

Same as script 39 (TESS survival analysis) with insolation bins:
  I < 10 Iearth  |  10 <= I < 50 Iearth  |  I >= 50 Iearth

Identical to script 39 in all science parameters; output goes to its own directory
so results from both scripts can coexist.

Usage:
    python scripts/12_survival_tess_cold.py
    python scripts/12_survival_tess_cold.py --rocky-mode radius-gap
    python scripts/12_survival_tess_cold.py --rocky-mode mass-confirmed
    python scripts/12_survival_tess_cold.py --rocky-mode radius-gap --star-types M
"""
...
OUT_DIR = ROOT / "output/plots" / "12_survival_tess_cold"
OUT_DIR.mkdir(parents=True, exist_ok=True)

# -- Paths (same as script 39) -------------------------------------------------
...
# Completeness grid (unchanged -- cells are finer than science bins)
...
def load_rocky_curve():
    ...
    print(f"  Rocky curve shift = {shift:+.4f} R_earth")
    return m, r, shift
...
def load_nasa(star_types: list[str], rocky_mode: str) -> pd.DataFrame:
    if not NASA_FLAGS_CACHE.exists():
        raise FileNotFoundError(f"NASA cache not found:\n  {NASA_FLAGS_CACHE}")
    ...
    rename = {
        ...
        "pl_bmassprov": "mass_provider", "pl_rade":      "radius_p",
        "pl_radeerr1":  "radius_err_plus","pl_radeerr2": "radius_err_minus",
        "pl_radelim":   "radius_limit_flag","pl_insol":   "flux_p",
        "st_teff":      "teff_s",
    }
...
def load_ppop_subset(n_catalogs: int) -> pd.DataFrame:
    ...
    print(f"  Loaded {len(files)} TESS P-Pop catalogs: {len(df):,} planets")
...
def assign_weights(nasa, eta):
    ...
    print(f"  Weights: min={weights.min():.2f}  med={np.median(weights):.2f}  "
          f"max={weights.max():.2f}  ({cap_frac:.0%} capped)")
    return weights


# -- 3. Statistics (identical to scripts 39/40) --------------------------------
...
def weighted_survival(radii, weights, r_grid):
    w_tot = weights.sum()
    return (np.array([weights[radii > rv].sum() / w_tot for rv in r_grid])
            if w_tot else np.zeros_like(r_grid))
...
def bootstrap_quantile_ci(radii, weights, q, n_boot, rng):
    ...
    n = len(radii)
    boot = [weighted_quantile(radii[idx := rng.integers(0, n, n)], weights[idx], q)
...
def bootstrap_count_ci(weights, n_boot, rng):
    ...
    if n == 0:
        return 0.0, 0.0, 0.0
    boot = [float(weights[rng.integers(0, n, n)].sum()) for _ in range(n_boot)]
    arr  = np.array(boot)
    return point, float(np.nanpercentile(arr, 2.5)), float(np.nanpercentile(arr, 97.5))
...
def permutation_test_count(w_low, w_high, n_perm, rng):
    ...
    n_low = len(w_low)
    null  = np.array([
        all_w[p := rng.permutation(len(all_w))][:n_low].sum() - all_w[p[n_low:]].sum()
        for _ in range(n_perm)
    ])
    return obs, float((null <= obs).mean()), null
...
def weighted_ks_upper(r_low, w_low, r_high, w_high, n_perm=2000, rng=None):
    ...
    mask_l, mask_h = r_low >= median_r, r_high >= median_r
...
def plot_completeness_grid(eta):
    ...
    im = ax.imshow(eta, origin="lower", aspect="auto", cmap="viridis", vmin=0, vmax=1,
    ...
    ax.set_yticklabels(
        [f"{v:.2f}" for v in 0.5 * (COMP_R_EDGES[:-1] + COMP_R_EDGES[1:])], fontsize=6)
    ax.set_xlabel("Insolation bin centre (Iearth)")
    ax.set_ylabel("Radius bin centre (Rearth)")
    ax.set_title("TESS P-Pop completeness grid  eta(R,I)\n"
                 f"Cold bin I < {INSOL_EDGES[1]:.0f} I_earth (spans left columns)")
    ...
    # Shade the columns that fall in the cold bin (I < INSOL_EDGES[1])
    cold_max_j = np.searchsorted(COMP_I_EDGES, INSOL_EDGES[1], side="left") - 1
    for j in range(cold_max_j + 1):
        ax.axvspan(j, j+1, alpha=0.12, color="steelblue")
    ax.text(cold_max_j / 2, len(COMP_R_EDGES) - 1.3,
            f"cold bin (I<{INSOL_EDGES[1]:.0f})", ha="center", fontsize=7, color="steelblue")
    fig.tight_layout()
...
def plot_survival_curves(groups, title_suffix):
    ...
    for ax, use_w, panel in [
        ...
        ax.grid(alpha=0.25)
    fig.suptitle(f"Survival curves (I<10 cold bin): planet radius by insolation bin\n{title_suffix}",
                 fontsize=11)
    fig.tight_layout()
...
def plot_bootstrap_quantiles(groups, title_suffix):
    ...
    ax.set_ylabel(f"Completeness-corrected Q90  [Rearth]")
    ax.set_title(f"Q90 by insolation bin (I<10 cold bin)  (95% bootstrap CI)\n{title_suffix}")
...
def plot_corrected_counts(groups, title_suffix, rocky_mode):
    ...
    ax.set_title(
        f"Corrected detection count (I<10 cold bin) -- rocky mode: {rocky_mode}\n"
...
def plot_permutation_null(obs, null, p_val, bins_compared, stat_label, fname):
    ...
    ax.set_title(
        f"Permutation test (I<10 cold bin): {bins_compared}\n"
...
def print_and_save_summary(nasa, weights, groups_data, perm_q90, perm_count,
    ...
    lines = [
        "=" * 72,
        "TESS (I<10 cold bin): ROCKY PLANET PRIMORDIAL FORMATION TEST",
        "=" * 72,
        f"COLD BIN  : I < {INSOL_EDGES[1]:.0f} I_earth",
    ...
    for la, lb, obs, pv in perm_q90:
        if np.isnan(pv):
            lines += [f"  {la} vs {lb}",
                      "    N/A (insufficient planets in cold bin for permutation test)"]
        else:
    ...
    for la, lb, D, p_ks in ks_results:
        if np.isnan(p_ks):
            lines.append(f"  {la} vs {lb}:  N/A (insufficient data)")
        else:
    ...
    lines += [
        "",
        "-- Scientific context -------------------------------------------",
        f"  Cold bin I < {INSOL_EDGES[1]:.0f} I_earth: XUV flux too low to strip envelopes.",
        "  radius-gap mode: R < 1.65 Rearth = primordially rocky (no stripping).",
        "=" * 72,
    ]
    text = "\n".join(lines)
    print(text)
...
def main():
    ...
    title_sfx  = (f"Stars: {'+'.join(star_types)}  |  rocky: {args.rocky_mode}"
                  f"  |  cold bin: I<{INSOL_EDGES[1]:.0f}  |  P-Pop: {PPOP_DATA_DIR.name}")
    ...
    print(f"\n-- 2. TESS P-Pop completeness ({args.n_catalogs} catalogs) ----------")
    ...
    if len(r_l) < MIN_BIN_N or len(r_h) < MIN_BIN_N:
        print(f"  SKIP: cold bin has {len(r_l)} planets, hot bin has {len(r_h)}.")
        print(f"  Need >= {MIN_BIN_N} per bin for pairwise tests.")
        perm_q90   = [(lbl_l, lbl_h, np.nan, np.nan), (lbl_l, lbl_m, np.nan, np.nan)]
        perm_count = [(lbl_l, lbl_h, np.nan, np.nan), (lbl_l, lbl_m, np.nan, np.nan)]
        ks_results = [(lbl_l, lbl_h, np.nan, np.nan), (lbl_l, lbl_m, np.nan, np.nan)]
    else:
        ...
        print(f"  Q90 cold vs hot:  delta={obs_q90_lh:+.4f}  p={pv_q90_lh:.4f}")
        print(f"  Q90 cold vs warm: delta={obs_q90_lm:+.4f}  p={pv_q90_lm:.4f}")
        plot_permutation_null(
            obs_q90_lh, null_q90_lh, pv_q90_lh,
            f"{lbl_l}  vs  {lbl_h}",
            "DeltaQ90 = Q90(cold) - Q90(hot)  [Rearth]",
        ...
        print(f"  Count cold vs hot: delta={obs_cnt_lh:+.1f}  p={pv_cnt_lh:.4f}")
        plot_permutation_null(
            obs_cnt_lh, null_cnt_lh, pv_cnt_lh,
            f"{lbl_l}  vs  {lbl_h}",
            "Delta_count = corrected_count(cold) - corrected_count(hot)",
