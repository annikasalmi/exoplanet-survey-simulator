...
class KeplerData:
    ...
    def estimate_cdpp_from_kepler_mag(self):
        ...
        Real Kepler rows still use rrmscdppXXpX columns when available.
        For P-Pop/Gaia rows with no real CDPP columns, this makes fainter
        approximate Kepler magnitudes noisier instead of using one flat CDPP.
        """
    ...
    def get_cdpp_ppm_for_duration(self, duration_hr):
        ...
        for i, label in enumerate(labels):
            ...
            cdpp_sources.append("kepmag_cdpp_fallback" if self.use_kepmag_cdpp_fallback else "fallback_cdpp_ppm")
...
# FUTURE UPGRADE 2:
# Replace n_transits >= 3 with the official Kepler window function.
# KeplerPORTs idea:
#     probdet = pipeline_detection_efficiency * window_function
# This is better because a period might have 3 theoretical transits but missing/bad cadences.
