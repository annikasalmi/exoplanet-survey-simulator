        ...
        if (Nplanets is None):
            ...
            for i in range(Nplanets):
                ...
                if (temp == 0):
                    ...
                    tempRp = rv.rvs(random_state=self.rng, Rbrk=self.Rbrk[1], alpha=self.alpha[0], norm=self.CR_less)[0] # Rearth
                    rv = rv_P(a=self.Porb_lims[0] / self.ytod, b=self.Porb_lims[1] / self.ytod)
                    tempPorb = rv.rvs(random_state=self.rng, beta=self.beta[0], norm=self.CP_less)[0] * self.ytod # d
                elif (temp == 1):
                    rv = rv_R(a=self.Rbrk[1], b=self.Rp_lims[1])
                    tempRp = rv.rvs(random_state=self.rng, Rbrk=self.Rbrk[1], alpha=self.alpha[1], norm=self.CR_more)[0] # Rearth
                    rv = rv_P(a=self.Porb_lims[0] / self.ytod, b=self.Porb_lims[1] / self.ytod)
                    tempPorb = rv.rvs(random_state=self.rng, beta=self.beta[1], norm=self.CP_more)[0] * self.ytod  # d
