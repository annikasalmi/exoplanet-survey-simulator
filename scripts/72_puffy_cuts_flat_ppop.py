"""
72_puffy_cuts_flat_ppop.py — puffy-fraction "which universe does NASA imply?" under four cuts,
...
Run:
    python scripts/72_puffy_cuts_flat_ppop.py
"""
...
OUT_DIR = os.path.join(ROOT, "my_outputs", "72_puffy_cuts_flat_ppop")
...
def mc_nasa(nasa, cut, m_sil, r_sil, rng):
    ...
    for i in range(N_REPEATS):
        ...
        if mb.size < 5:
            continue
        out[i] = puffy_frac(mb, rb, m_sil, r_sil)
    return out[np.isfinite(out)], n
