"""
60_cold_corner_metric_power.py — CAN we prove/disprove rocky planets in the cold top-left
...
Run:
    python scripts/60_cold_corner_metric_power.py
"""
...
OUT_DIR = os.path.join(ROOT, "my_outputs", "60_cold_corner_metric_power")
...
def main():
    ...
    axc.text(0.2, 0.92, "COLD corner", color="tab:blue", fontsize=10, transform=axc.get_xaxis_transform())
    ...
    axp.bar(x - 0.2, [power["cold"][m] for m in mk_labels], 0.4, label=f"COLD corner (flux<{COLD_FLUX:g})", color="tab:blue")
