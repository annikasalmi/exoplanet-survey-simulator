"""
71_universe_mr_scatter_cuts.py — SEE the sampled planet populations on the mass-radius diagram
...
Run:
    python scripts/71_universe_mr_scatter_cuts.py
"""
...
OUT_DIR = os.path.join(ROOT, "my_outputs", "71_universe_mr_scatter_cuts")
...
def main():
    ...
    # ---- figure: 2x2 M-R scatter ----
    fig, axes = plt.subplots(2, 2, figsize=(15, 11), sharex=True, sharey=True)
    for ax, (cut_label, cut) in zip(axes.flat, CUTS):
        # silicate curve
        ax.plot(m_sil, r_sil, "k-", lw=2.5, zorder=6, label="silicate rock limit")
        ax.fill_between(m_sil, r_sil, 2.6, color="0.92", zorder=0)   # puffy region shading (above)
        title_bits = []
        for pop, drop, label, colour in UNIVERSES:
            ...
            if mo.size > N_PLOT:
                ...
                mo, ro = mo[j], ro[j]
            ax.scatter(mo, ro, s=7, color=colour, alpha=0.35, lw=0, label=f"{label} (puffy {pf:.2f})")
        nmc, nrc = nasa_cut(nm, nr, nins, cut)
        pf_nasa = puffy_fraction(nmc, nrc, m_sil, r_sil)
        ax.scatter(nmc, nrc, s=26, facecolor="none", edgecolor="k", lw=0.9, zorder=5,
    ...
    fig.suptitle("What each universe's sampled population looks like on the mass-radius diagram (transit+RV detected + measurement noise)\n"
                 "points above the black silicate curve = puffy, below = rocky; NASA = open black circles. The puffy fraction is just the fraction above the curve.",
                 fontsize=12)
    fig.tight_layout(rect=[0, 0, 1, 0.92])
    out_png = os.path.join(OUT_DIR, "universe_mr_scatter_cuts.png")
