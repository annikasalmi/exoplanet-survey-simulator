"""
mc_mr_density_priors.py — 2x3 mass-radius probability-density map built from 4,000 Monte
Carlo noise realizations of each of the two priors in bayesian_cold_rocky_desert.py, so the
two priors' predicted mass-radius densities (with realistic measurement scatter) can be
compared side by side, panel by panel.

Same noise model as predicted_frac() in bayesian_cold_rocky_desert.py (the function that
...
Run:
    python important_plots/mc_mr_density_priors.py
"""
...
def mc_density(pool, lo, hi, rng, mass_min=None, n_mc=N_MC, batch=BATCH):
    ...
    bayesian_cold_rocky_desert.py — boundary planets can flip in/out of the cut draw to draw,
