"""
44_2x4_kepler_tess_rocky-fgkm-detection.py
...
Run from repo root:
    python scripts/44_2x4_kepler_tess_rocky-fgkm-detection.py
"""
...
KEPLER_PPOP_DIR = ROOT / "run" / "kepler" / "data" / "Gaia_C_F_K_combined"
TESS_PPOP_DIR   = ROOT / "run" / "tess"   / "data" / "Gaia_C_F_K_combined_cdpp_v1"
...
OUT_DIR = ROOT / "my_outputs" / "44_2x4_kepler_tess_rocky-fgkm-detection"
...
def load_kepler_ppop() -> pd.DataFrame:
    files = sorted(KEPLER_PPOP_DIR.glob("kepler_catalog_*.csv"))
    if not files:
    ...
    print(f"Loaded {len(files)} Kepler P-Pop file(s): {len(df):,} rows")
...
def load_tess_ppop() -> pd.DataFrame:
    ...
    for p in files:
        ...
        frames.append(d)
    df = pd.concat(frames, ignore_index=True)
    print(f"Loaded {len(files)} TESS P-Pop file(s): {len(df):,} rows")
...
def plot_combined(kepler: pd.DataFrame, tess: pd.DataFrame,
    ...
    fig.suptitle(
        ...
        f"(anchored at LHS 1140 b)  |  red error bars = two-sided radius uncertainty",
        fontsize=13,
    )
...
def plot_mr_diagnostic(m_ref, r_ref, shift: float,
    ...
    ax.plot(m_line, r_rocky,     "k--", lw=1.5, label="Pure-rock curve (ref.ddat)", zorder=5)
    ax.plot(m_line, r_threshold, color="crimson", lw=2.0,
            label=f"Rocky threshold (+{shift:.3f} R⊕)", zorder=5)
    ...
    ax.set_title("Rocky threshold diagnostic — pure-rock curve shifted to pass through LHS 1140 b",
                 fontsize=11)
...
def main():
    print("=" * 70)
    print("44_2x4_kepler_tess_rocky-fgkm-detection.py")
    print("=" * 70)
    print(f"Project root  : {ROOT}")
    print(f"Kepler P-Pop  : {KEPLER_PPOP_DIR}")
    print(f"TESS   P-Pop  : {TESS_PPOP_DIR}")
