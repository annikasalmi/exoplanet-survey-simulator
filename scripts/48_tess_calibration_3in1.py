...
Outputs:
    my_outputs/48_tess_calibration_3in1/
        tess_3in1_calibration.png
        toi_cached.csv
        summary.txt

Run from repo root:
    python scripts/48_tess_calibration_3in1.py
"""
...
OUT_DIR = ROOT / "my_outputs" / "48_tess_calibration_3in1"
...
def make_3in1(out: pd.DataFrame) -> None:
    ...
    ax_scatter.set_xlabel("Official TESS SNR (ExoFOP/SPOC)")
    ax_scatter.set_ylabel("Toy TESS SNR")
    ax_scatter.set_title("A. Toy TESS SNR vs official SNR")
    ...
    ax_recov.set_ylabel("Toy TESS pass fraction"); ax_recov.set_xlabel("Official SPOC SNR bin")
    ...
    ax_ratio.set_ylabel("Toy SNR / official SNR"); ax_ratio.set_xlabel("Official SPOC SNR bin")
    ax_ratio.set_title("C. SNR ratio: raw vs calibrated (below 1 = conservative)")
    ax_ratio.legend(fontsize=7); ax_ratio.grid(axis="y", alpha=0.25)

    fig.suptitle(
        "TESS 3-in-1 calibration: toy TESS detector vs official SNR\n"
        "Improvement: smooth noise reference 60->30 ppm/hr, floor 30->10 ppm/hr",
        fontsize=11, y=1.01
    )
