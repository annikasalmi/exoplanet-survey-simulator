...
Outputs (single combined figure):
    my_outputs/47_kepler_calibration_3in1/
...
Run from repo root:
    python scripts/47_kepler_calibration_3in1.py
"""
...
OUT_DIR = ROOT / "my_outputs" / "47_kepler_calibration_3in1"
...
def make_3in1(out: pd.DataFrame) -> None:
    ...
    ax_scatter.set_xlabel("Official Kepler KOI MES (koi_max_mult_ev)")
    ax_scatter.set_ylabel("Toy Kepler MES (kepler_mes)")
    ax_scatter.set_title("A. Toy MES vs official KOI MES\n(with real rrmscdpp CDPP)")
    ...
    ax_recov.set_ylabel("Toy Kepler pass fraction"); ax_recov.set_xlabel("Official KOI MES bin")
    ax_recov.set_title("B. Recovery rises with signal strength")
    ...
    ax_ratio.set_ylabel("Toy MES / official KOI MES"); ax_ratio.set_xlabel("Official KOI MES bin")
    ax_ratio.set_title("C. MES ratio: raw vs calibrated (below 1 = conservative)")
    ax_ratio.legend(fontsize=7); ax_ratio.grid(axis="y", alpha=0.25)

    # ── Save ─────────────────────────────────────────────────────────────────
    fig.suptitle(
        "Kepler 3-in-1 calibration: toy KeplerData vs official KOI DR25 MES\n"
        "Improvement: rrmscdpp* CDPP from stellar table (replaces magnitude fallback)",
        fontsize=11, y=1.01
    )
