"""
71_puffy_pipeline_table.py — clean one-page table explaining, step by step, how the
...
Run:
    python scripts/71_puffy_pipeline_table.py
"""
...
OUT_DIR = os.path.join(ROOT, "my_outputs", "71_puffy_pipeline_table")
...
ROWS = [
    ("1 · Population",
     ...
     "292 real confirmed + transiting planets with a MEASURED mass (RV/TTV). No pool built."),
    ...
    ("3 · Sample\n(one draw)",
     ...
     "Bootstrap: resample the 292 planets with replacement."),
    ...
    ("7 · Repeat",
     ...
     "Redo the bootstrap 10,000 times → 10,000 values of p."),

    ("8 · Fit μ, σ",
     "μ = mean(p),  σ = std(p).   By the CLT:  σ ≈ √(p(1−p) / N_det).",
     "Same; σ ≈ √(p(1−p) / 292) — narrower than the models because 292 > N_det (~150)."),
